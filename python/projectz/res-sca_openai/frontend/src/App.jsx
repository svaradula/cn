...
export default function App() {
  ...
  // ── Check MS auth on mount + handle callback redirect ─────────────────────
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("ms_auth") === "success") {
      const email = params.get("user_email") || "";
      setMsAuthenticated(true);
      setHrEmail(email);
      showToast("Connected to Microsoft Outlook ✓", "success");
      // Clean URL
      ...
      fetch(`${import.meta.env.VITE_API_BASE_URL}/api/auth/status`)
        .then((r) => r.json())
        .then((d) => {
          setMsAuthenticated(d.authenticated);
          setHrEmail(d.user_email || "");
        })
        .catch(() => {});
    }
  }, []);
  ...
  const handleScheduled = (candidate, scheduleDetails) => {
    ...
    // Toast
    showToast("Interview scheduled successfully and Teams invite sent. ✓", "success");
  };
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {msAuthenticated ? (
            <div className="flex items-center gap-1.5 text-xs text-green-600 font-medium
              ...
              {hrEmail || "Outlook connected"}
            </div>
          ) : (
            <button
              onClick={() => window.location.href = `${import.meta.env.VITE_API_BASE_URL}/api/auth/login`}
      ...
      {scheduleModalOpen && scheduleTarget && (
        <ScheduleModal
          ...
          roleName={roleName}
          hrEmail={hrEmail}
          isAuthenticated={msAuthenticated}
