...
const LEVEL_CONFIG = {
  basic: {
    ...
    sectionBorder: "#dbeafe", sectionBg: "#eff6ff",
    ...
    answerBg: "#f0f7ff", answerBorder: "#bfdbfe", toggleColor: BRAND.navy,
  },
  intermediate: {
    label: "Intermediate",
    ...
    sectionBorder: "#ede8fd", sectionBg: "#f9f5ff",
    ...
    tagJD:     { background: "#ede8fd", color: BRAND.purple, border: "1px solid #c4b0f0" },
    tagResume: { background: "#fde8f0", color: BRAND.pink,   border: "1px solid #f4b8d4" },
    answerBg: "#faf5ff", answerBorder: "#c4b0f0", toggleColor: BRAND.purple,
  },
  advanced: {
    label: "Advanced",
    ...
    sectionBorder: "#fde8f0", sectionBg: "#fff5f9",
    ...
    answerBg: "#fff8f5", answerBorder: "#f4c9a8", toggleColor: BRAND.pink,
  },
};
...
export default function QuestionModal({
  ...
  isLoading,
  error,
  score,            // match score from shortlist (passed through for Phase 4)
  onClose,
  onShortlist,      // (filename, candidate, score) → adds to Phase 4
  isShortlisted,    // boolean — already in Phase 4?
}) {
  const overlayRef = useRef(null);
  // Local toggle state — initialised from prop so re-opening shows correct state
  const [shortlisted, setShortlisted] = useState(isShortlisted ?? false);
  ...
  const handleShortlistToggle = () => {
    ...
    setShortlisted(next);
    if (next) {
      // Add to Phase 4 and close modal
      onShortlist(filename, candidate, score);
      onClose();
    } else {
      // Un-shortlist — just toggle state, parent handles removal via Phase 4 remove button
      onShortlist(filename, null, score);
    }
  };
        ...
        {!isLoading && (
          <div className="border-t border-gray-100 px-6 py-4 shrink-0 bg-gray-50">
            <div className="flex items-center justify-between gap-4">
              ...
              {/* Right: shortlist toggle button */}
              <ShortlistButton
                shortlisted={shortlisted}
                onToggle={handleShortlistToggle}
...
function ShortlistButton({ shortlisted, onToggle, disabled }) {
  return (
    <button
      onClick={onToggle}
      disabled={disabled}
      className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold
                 text-xs transition-all duration-200 shrink-0"
      style={
        shortlisted
          ? {
              background: "#f0fdf4",
    ...
    <div className="rounded-xl p-4 flex items-center gap-4"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
          <DetailChip icon={<EmailIcon />} value={candidate.email}
            href={notFound(candidate.email) ? null : `mailto:${candidate.email}`} />
          <DetailChip icon={<PhoneIcon />} value={candidate.phone}
...
function DetailChip({ icon, value, href }) {
  ...
  if (href) {
    return (
      <a href={href} className="flex items-center gap-1.5 text-xs transition-opacity hover:opacity-70"
         style={{ color: BRAND.purple }} onClick={(e) => e.stopPropagation()}>
        <span className="shrink-0">{icon}</span>{value}
      </a>
    );
  }
  return (
...
function LevelSection({ section, defaultOpen }) {
  ...
  return (
    <div className="rounded-xl" style={{ border: `1px solid ${config.sectionBorder}` }}>
    ...
    <li className="px-4 py-4 flex flex-col gap-2">
      <div className="flex gap-3">
        <div className="shrink-0 w-6 h-6 rounded-full border flex items-center
                        justify-center text-xs font-bold mt-0.5" style={config.indexStyle}>
          {index}
        </div>
        <div className="flex-1 flex flex-col gap-1.5">
          ...
          {hasAnswer && (
            <button onClick={() => setAnswerOpen((o) => !o)}
...
function CheckIcon({ size = 12 }) {
  return (
    <svg style={{ width: size, height: size }} fill="none" viewBox="0 0 24 24"
